...
impl Repository<Promocao> for PromocaoRepository {
    ...
    async fn criar(&self, item: &Promocao) -> Result<Uuid, String> {
        sqlx::query("
            INSERT INTO promocoes (uuid, loja_uuid, nome, descricao, tipo_desconto, valor_desconto, valor_minimo, data_inicio, data_fim, dias_semana_validos, tipo_escopo, produto_uuid, categoria_uuid, prioridade, status)
