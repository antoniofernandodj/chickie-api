...
impl Repository<Loja> for LojaRepository {
    ...
    async fn criar(&self, item: &Loja) -> Result<Uuid, String> {
        sqlx::query("
            INSERT INTO lojas (uuid, nome, slug, descricao, email, telefone, ativa, logo_url, banner_url, horario_abertura, horario_fechamento, dias_funcionamento, tempo_preparo_min, taxa_entrega, valor_minimo_pedido, raio_entrega_km, criado_em, atualizado_em)
    ...
    async fn atualizar(&self, item: Loja) -> Result<(), String> {
        ...
        let result = sqlx::query("
            UPDATE lojas SET nome = $1, slug = $2, descricao = $3, email = $4, telefone = $5, ativa = $6, logo_url = $7, banner_url = $8, horario_abertura = $9, horario_fechamento = $10, dias_funcionamento = $11, tempo_preparo_min = $12, taxa_entrega = $13, valor_minimo_pedido = $14, raio_entrega_km = $15, atualizado_em = $16
            WHERE uuid = $17
        ")
        .bind(&item.nome)
        .bind(&item.slug)
