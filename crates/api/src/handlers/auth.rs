...
/// Reutilizável tanto no middleware HTTP quanto no handshake WebSocket.
pub async fn validar_token(token: &str, state: &AppState) -> Result<Usuario, AppError> {
    ...
    let token_data = decode::<Claims>(
        ...
        AppError::Unauthorized("Token inválido ou expirado. Faça login novamente.".into())
    })?;

    let user_uuid = token_data.claims.sub.parse().map_err(|_|
        AppError::Unauthorized("Token mal formado: UUID do usuário inválido".into())
    )?;
    ...
    let usuario = state.usuario_repo
        ...
        .map_err(|e| {
            ...
            AppError::Internal("Erro interno ao validar usuário".into())
        })?
        .ok_or_else(|| AppError::Unauthorized("Usuário do token não encontrado no banco de dados".into()))?;

    if usuario.esta_bloqueado() {
        return Err(AppError::Unauthorized("Usuário bloqueado. Contate o suporte.".into()));
    }
...
pub async fn auth_middleware(
    ...
    let usuario = validar_token(token, &state).await.map_err(|e| {
        let (status, msg) = match e {
            AppError::Unauthorized(m) => (StatusCode::UNAUTHORIZED, m),
            AppError::Internal(m) => (StatusCode::INTERNAL_SERVER_ERROR, m),
            _ => (StatusCode::UNAUTHORIZED, "Erro de autenticação".into()),
        };
